...
// Work-timer v7: edit removed, summary excludes zero-premium items
...
function render(){
  ...
  // list only non-zero premiums in details
  const bonusItems = monthEntries.filter(x => x.bonus && x.bonus > 0);
  let bonusHtml = '';
  if(bonusItems.length > 0){
    bonusHtml = `<details style="margin-top:6px;"><summary>Premie: ${totalBonus.toFixed(2)} PLN</summary><ul>${bonusItems.map(e=>`<li>${(e.note||'-')}: ${(e.bonus||0).toFixed(2)} PLN</li>`).join('')}</ul></details>`;
  } else {
    bonusHtml = `<div>Premie: 0.00 PLN</div>`;
  }
